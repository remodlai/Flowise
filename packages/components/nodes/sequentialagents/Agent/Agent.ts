...
import { CustomChainHandler, additionalCallbacks, ConsoleCallbackHandler } from '../../../src/handler'
...
async function agentNode(
    ...
    try {
        ...
        const shouldStreamResponse = options.shouldStreamResponse || true
        ...
        if (shouldStreamResponse && sseStreamer) {
            ...
            result = await agent.invoke({ ...state, signal: abortControllerSignal.signal, callbacks: [loggerHandler, handler, ...callbacks] }, config)
            for (const chunk of result) {
                streamResponse(sseStreamer, chatId, chunk)
            }
