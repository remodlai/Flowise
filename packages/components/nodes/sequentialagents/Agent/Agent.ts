...
async function agentNode(
    ...
    try {
        ...
        // let result = {
        //     content: '',
        //     usedTools: [] as IUsedTool[],
        //     sourceDocuments: [] as IDocument[],
        //     artifacts: [] as ICommonObject[],
        //     additional_kwargs: {} as ICommonObject,
        //     name: '',
        //     output: ''
        // }
       ...
       isStreamingStarted = false
        // Get the iterator for streaming
        const iterator = agent._streamIterator({ ...state, signal: abortControllerSignal.signal, callbacks: [handler, ...callbacks] }, config)
        ...
        for await (const chunk of iterator) {
            ...
            if (typeof chunk === 'string') {
                // For string chunks, both stream and accumulate
                result.content += chunk
                result.output = result.content
                if (shouldStreamResponse && sseStreamer && !isStreamingStarted) {
                    
                    sseStreamer.streamTokenEvent(chatId, chunk)
                }
            } else if (chunk) {
                // For object chunks
                if (chunk.content) {
                    ...
                    result.output = result.content
                    if (shouldStreamResponse && sseStreamer) {
                        // Only stream content if it's not a tool call
                        const isToolCall = chunk.additional_kwargs?.tool_calls || 
                                         (chunk.message && chunk.message.tool_calls) ||
                                         (chunk.message && chunk.message.tool_call_chunks)
                        if (!isToolCall) {
                ...
                if (Array.isArray(chunk.usedTools)) result.usedTools.push(...chunk.usedTools)
                if (Array.isArray(chunk.sourceDocuments)) result.sourceDocuments.push(...chunk.sourceDocuments)
                if (Array.isArray(chunk.artifacts)) result.artifacts.push(...chunk.artifacts)
                if (chunk.additional_kwargs) result.additional_kwargs = { ...result.additional_kwargs, ...chunk.additional_kwargs }
            }
        }

        // End the stream if we were streaming
        if (shouldStreamResponse && sseStreamer) {
        ...
        if (interrupt) {
            ...
            if (lastMessage && lastMessage._getType() === 'tool' && lastMessage.additional_kwargs?.nodeId === nodeData.id) {
                result = {
                    ...
                    usedTools: lastMessage.additional_kwargs?.usedTools as IUsedTool[] || [],
                    sourceDocuments: lastMessage.additional_kwargs?.sourceDocuments as IDocument[] || [],
                    artifacts: lastMessage.additional_kwargs?.artifacts as ICommonObject[] || [],
                    additional_kwargs: {},
                    name: '',
