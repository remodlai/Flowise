...
class File_DocumentLoaders {
    ...
    async init(nodeData, _, options) {
        ...
        if (totalFiles.startsWith('FILE-STORAGE::')) {
            ...
            if (retrieveAttachmentChatId) {
                for (const file of files) {
                    if (!file)
                        continue;
                    const fileData = await (0, storageUtils_1.getFileFromStorage)(file, chatflowid, options.chatId);
                ...
                for (const file of files) {
                    if (!file)
                        continue;
                    const fileData = await (0, storageUtils_1.getFileFromStorage)(file, chatflowid);
                    const blob = new Blob([fileData]);
                    fileBlobs.push({ blob, ext: file.split('.').pop() || '' });
                }
            }
        ...
        const loader = new MultiFileLoader(fileBlobs, {
            ...
            xls: (blob) => new csv_1.CSVLoader(blob),
            xlsx: (blob) => new csv_1.CSVLoader(blob),
            docx: (blob) => new docx_1.DocxLoader(blob),
            doc: (blob) => new docx_1.DocxLoader(blob),
            pdf: (blob) => pdfUsage === 'perFile'
                ? // @ts-ignore
                    new pdf_1.PDFLoader(blob, { splitPages: false, pdfjs: () => Promise.resolve().then(() => __importStar(require('pdf-parse/lib/pdf.js/v1.10.100/build/pdf.js'))) })
                : // @ts-ignore
                    new pdf_1.PDFLoader(blob, { pdfjs: () => Promise.resolve().then(() => __importStar(require('pdf-parse/lib/pdf.js/v1.10.100/build/pdf.js'))) }),
...
const getOverrideFileInputs = (nodeData) => {
