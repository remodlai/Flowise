...
const getAllApiKeys = async (req, res, next) => {
    try {
        const apiResponse = await apikey_1.default.getAllApiKeys(req);
...
const createApiKey = async (req, res, next) => {
    try {
        ...
        const apiResponse = await apikey_1.default.createApiKey(req.body.keyName, req);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const updateApiKey = async (req, res, next) => {
    try {
        ...
        const apiResponse = await apikey_1.default.updateApiKey(req.params.id, req.body.keyName, req);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const deleteApiKey = async (req, res, next) => {
    try {
        ...
        const apiResponse = await apikey_1.default.deleteApiKey(req.params.id, req);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
