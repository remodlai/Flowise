...
const getNodesByCategory = async (req, res, next) => {
    try {
        if (typeof req.params.name === 'undefined' || req.params.name === '') {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: nodesController.getNodesByCategory - name not provided!`);
        }
        const name = lodash_1.default.unescape(req.params.name);
        const apiResponse = await nodes_1.default.getAllNodesForCategory(name);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
