...
const getSingleOpenaiAssistant = async (credentialId, assistantId) => {
    try {
        ...
        if (dbResponse.tool_resources?.file_search?.vector_store_ids?.length) {
            ...
            dbResponse.tool_resources.file_search.vector_store_object = await openai.beta.vectorStores.retrieve(vectorStoreId);
        }
        return dbResponse;
    }
    catch (error) {
...
const uploadFilesToAssistant = async (credentialId, files) => {
    ...
    for (const file of files) {
        const fileBuffer = await (0, storageUtils_1.getFileFromUpload)(file.filePath);
        ...
        await (0, storageUtils_1.removeSpecificFileFromUpload)(file.filePath);
    }
