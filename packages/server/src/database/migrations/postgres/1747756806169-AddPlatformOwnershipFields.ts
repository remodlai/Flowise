 ...
 * 
 * This migration adds platform ownership fields to multiple entities in the system to support
 * multi-tenancy and organization-level access control. The main changes include:
 * 
 * 1. Adding ownership fields to 9 entities:
 *    - applicationId (uuid NOT NULL): All entities are now associated with an application
 *    - organizationId (uuid, nullable): Optional organization ownership
 *    - userId (uuid, nullable): Optional user ownership
 * 
 * 2. Special handling for NOT NULL constraints on existing tables:
 *    Since adding NOT NULL columns to tables with existing data would fail, this migration
 *    uses a 3-step process for each entity:
 *    a. Add the column as nullable
 *    b. Update existing rows with default application ID '3b702f3b-5749-4bae-a62e-fb967921ab80'
 *    c. Add the NOT NULL constraint after data is populated
 * 
 * 3. Other schema changes:
 *    - Various column type changes (varchar to text)
 *    - Added constraints, indices and foreign keys
 *    - Other field modifications
 * 
 * This migration is designed to be safely applied to existing databases with data
...
export class AddPlatformOwnershipFields1747756806169 implements MigrationInterface {
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        try {
            ...
            // ===== Remove Platform Ownership Fields =====
            // CustomTemplate
