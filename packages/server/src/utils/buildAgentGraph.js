...
const handler_1 = require("@components/handler");
 ...
 * Build Agent Graph
 */
const buildAgentGraph = async ({ agentflow, flowConfig, incomingInput, nodes, edges, initializedNodes, endingNodeIds, startingNodeIds, depthQueue, chatHistory, uploadedFilesContent, appDataSource, componentNodes, sseStreamer, shouldStreamResponse, cachePool, baseURL, signal }) => {
    try {
        ...
        if (shouldStreamResponse && sseStreamer && chatId) {
            ...
            class DirectTokenStreamingHandler extends base_1.BaseCallbackHandler {
                constructor(chatId, sseStreamer, filterNodeTypes) {
                    super();
                    this.chatId = chatId;
                    this.sseStreamer = sseStreamer;
                    this.filterNodeTypes = filterNodeTypes;
                    this.name = 'direct_token_streaming_handler';
                ...
                handleLLMNewToken(token, options) {
            ...
            const nodeTypesToFilter = ['seqConditionAgent', 'seqCondition']; // e.g. ['seqCustomFunction', 'seqToolNode']
        ...
        try {
                        ...
                        for (const agentName of Object.keys(output)) {
                            ...
                            if (shouldStreamResponse) {
                                ...
                                if (shouldStreamResponse && options.callbacks.length > 0) {
                                    ...
                                    if (tokenHandler && typeof tokenHandler.setCurrentNodeType === 'function') {
                                        tokenHandler.setCurrentNodeType(nodeName);
                                    }
                                }
                            }
                        }
            ...
            await (0, utils_1.clearSessionMemory)(nodes, componentNodes, chatId, appDataSource, sessionId);
...
const compileMultiAgentsGraph = async (params) => {
    ...
    for (const supervisor in supervisorWorkers) {
        ...
        try {
            ...
            const loggerHandler = new handler_1.ConsoleCallbackHandler(logger_1.default);
            const callbacks = await (0, handler_1.additionalCallbacks)(flowNodeData, options);
