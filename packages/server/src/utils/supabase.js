"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.requireOrgAdmin = exports.requirePlatformAdmin = exports.isAppOwner = exports.isOrgAdmin = exports.isPlatformAdmin = exports.checkUserRole = exports.supabase = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
const index_1 = require("../index");
const logger_1 = __importDefault(require("./logger"));
// DEPRECATED: Direct client initialization
// This is kept for backward compatibility but will be removed in a future version
// Use the centralized client from App class instead
const supabaseUrl = process.env.SUPABASE_URL || '';
const supabaseServiceKey = process.env.SUPABASE_SERVICE_KEY || '';
// For backward compatibility, we'll still export a supabase client
// But we'll try to get it from the App instance first
let _supabase;
// This function gets the Supabase client from the App instance if available,
// or falls back to the direct client if not
const getSupabaseClient = () => {
    try {
        const app = (0, index_1.getInstance)();
        if (app && app.Supabase) {
            return app.Supabase;
        }
    }
    catch (error) {
        logger_1.default.warn('[supabase]: Could not get Supabase client from App instance, falling back to direct client');
    }
    // Fallback to direct client if App instance is not available
    if (!_supabase) {
        _supabase = (0, supabase_js_1.createClient)(supabaseUrl, supabaseServiceKey);
    }
    return _supabase;
};
// Export a proxy that will always use the latest client
exports.supabase = new Proxy({}, {
    get: (target, prop) => {
        const client = getSupabaseClient();
        return client[prop];
    }
});
// Authorization utility functions
const checkUserRole = async (userId, role, entityId) => {
    try {
        logger_1.default.debug(`Checking if user ${userId} has role ${role} ${entityId ? `for entity ${entityId}` : ''}`);
        // First, get the role ID for the specified role
        const { data: roleData, error: roleError } = await exports.supabase
            .from('roles')
            .select('id')
            .eq('name', role)
            .maybeSingle();
        if (roleError) {
            logger_1.default.error('Error fetching role:', roleError);
            return false;
        }
        if (!roleData) {
            logger_1.default.error(`Role '${role}' not found`);
            return false;
        }
        // Now check if the user has this role
        let query = exports.supabase
            .from('user_roles')
            .select('id')
            .eq('user_id', userId)
            .eq('role_id', roleData.id);
        // If entityId is provided, filter by resource_id
        if (entityId) {
            query = query.eq('resource_id', entityId);
        }
        const { data, error } = await query.maybeSingle();
        if (error) {
            logger_1.default.error(`Error checking role:`, error);
            return false;
        }
        // For platform_admin, always return true if they have the role
        // regardless of entity
        if (role === 'platform_admin' && data) {
            return true;
        }
        return !!data;
    }
    catch (error) {
        logger_1.default.error(`Error checking ${role} status:`, error);
        return false;
    }
};
exports.checkUserRole = checkUserRole;
// Check if user is platform admin
const isPlatformAdmin = async (userId) => {
    return (0, exports.checkUserRole)(userId, 'platform_admin');
};
exports.isPlatformAdmin = isPlatformAdmin;
// Check if user is org admin for a specific org
const isOrgAdmin = async (userId, orgId) => {
    return (0, exports.checkUserRole)(userId, 'org_admin', orgId);
};
exports.isOrgAdmin = isOrgAdmin;
// Check if user is app owner for a specific app
const isAppOwner = async (userId, appId) => {
    // For platform admins, always return true
    const isUserPlatformAdmin = await (0, exports.isPlatformAdmin)(userId);
    if (isUserPlatformAdmin) {
        return true;
    }
    // For regular users, check if they have app_admin role
    // Note: We're using app_admin instead of app_owner since that's what exists in our schema
    return (0, exports.checkUserRole)(userId, 'app_admin', appId);
};
exports.isAppOwner = isAppOwner;
// Middleware to check if user is platform admin
const requirePlatformAdmin = async (req, res, next) => {
    const user = req.user;
    if (!user || !user.userId) {
        res.status(401).json({ error: 'Unauthorized' });
        return;
    }
    const isAdmin = await (0, exports.isPlatformAdmin)(user.userId);
    if (!isAdmin) {
        res.status(403).json({ error: 'Forbidden - Not a platform admin' });
        return;
    }
    next();
};
exports.requirePlatformAdmin = requirePlatformAdmin;
// Middleware to check if user is org admin
const requireOrgAdmin = (orgId) => async (req, res, next) => {
    const user = req.user;
    const targetOrgId = orgId || req.params.orgId || req.body.orgId;
    if (!user || !user.userId) {
        res.status(401).json({ error: 'Unauthorized' });
        return;
    }
    if (!targetOrgId) {
        res.status(400).json({ error: 'Organization ID is required' });
        return;
    }
    const isAdmin = await (0, exports.isOrgAdmin)(user.userId, targetOrgId);
    if (!isAdmin) {
        res.status(403).json({ error: 'Forbidden - Not an organization admin' });
        return;
    }
    next();
};
exports.requireOrgAdmin = requireOrgAdmin;
//# sourceMappingURL=supabase.js.map