 ...
 */
const buildFlow = async ({ startingNodeIds, reactFlowNodes, reactFlowEdges, graph, depthQueue, componentNodes, question, uploadedFilesContent, chatHistory, apiMessageId, chatId, sessionId, chatflowid, appDataSource, overrideConfig, apiOverrideStatus = false, nodeOverrides = {}, availableVariables = [], variableOverrides = [], cachePool, isUpsert, stopNodeId, uploads, baseURL, appId, orgId, userId }) => {
    logger_1.default.debug('========= Start of buildFlow (file: packages/server/src/utils/index.ts LINE: 497) =========');
    logger_1.default.debug(`appId: ${appId || 'not provided'}`);
    logger_1.default.debug(`orgId: ${orgId || 'not provided'}`);
    logger_1.default.debug(`userId: ${userId || 'not provided'}`);
    logger_1.default.debug(`startingNodeIds: ${JSON.stringify(startingNodeIds)}`);
    logger_1.default.debug(`reactFlowNodes count: ${reactFlowNodes.length}`);
    logger_1.default.debug(`reactFlowEdges count: ${reactFlowEdges.length}`);
    logger_1.default.debug('========= End of buildFlow parameter logging =========');
    ...
    logger_1.default.debug('========= Start of buildFlow initializedNodes (file: packages/server/src/utils/index.ts LINE: 500) =========');
    logger_1.default.debug(`'initializedNodes', ${JSON.stringify(initializedNodes)}`);
    logger_1.default.debug('========= End of buildFlow initializedNodes (file: packages/server/src/utils/index.ts LINE: 500) =========');
    ...
    logger_1.default.debug(`========= Start of flowData =========`);
    logger_1.default.debug('  ');
    logger_1.default.debug(`flowData, ${JSON.stringify(flowData)}`);
    logger_1.default.debug(`========= End of flowData =========`);
    logger_1.default.debug('  ');
    while (nodeQueue.length) {
        ...
        try {
            ...
            logger_1.default.debug('  ');
            logger_1.default.debug(`Current nodeModule: ${JSON.stringify(nodeModule)}`);
            logger_1.default.debug('  ');
            const newNodeInstance = new nodeModule.nodeClass();
            logger_1.default.debug('  ');
            logger_1.default.debug(`Current newNodeInstance: ${JSON.stringify(newNodeInstance)}`);
            //logger.debug('  ')
            ...
            logger_1.default.debug('  ');
            logger_1.default.debug(`'reactFlowNode', ${JSON.stringify(reactFlowNode.data)}`);
            let flowNodeData = (0, lodash_1.cloneDeep)(reactFlowNode.data);
            //REMODL: Logging for each flowNodeData
            logger_1.default.debug(`'flowNodeData', ${JSON.stringify(flowNodeData)}`);
            ...
            if (isUpsert && stopNodeId && nodeId === stopNodeId) {
                ...
                logger_1.default.debug(`Initializing node: ${reactFlowNode.data.label} (${reactFlowNode.data.id})`);
                logger_1.default.debug(`Node credential ID: ${reactFlowNode.data.credential ?? 'none'}`);
                logger_1.default.debug(`Passing appId: ${appId || 'none'} to init method`);
    ...
    while (startIdx < endIdx) {
        ...
        if (substr === '}}' && variableStack.length > 0 && variableStack[variableStack.length - 1].substr === '{{') {
            ...
            if (isAcceptVariable && variableFullPath === QUESTION_VAR_PREFIX) {
                variableDict[`{{${variableFullPath}}}`] = (0, utils_1.handleEscapeCharacters)(question, false);
            }
            if (isAcceptVariable && variableFullPath === FILE_ATTACHMENT_PREFIX) {
                variableDict[`{{${variableFullPath}}}`] = (0, utils_1.handleEscapeCharacters)(uploadedFilesContent, false);
            }
            if (isAcceptVariable && variableFullPath === CHAT_HISTORY_VAR_PREFIX) {
                variableDict[`{{${variableFullPath}}}`] = (0, utils_1.handleEscapeCharacters)((0, utils_1.convertChatHistoryToText)(chatHistory), false);
            }
            ...
            if (executedNode) {
                ...
                if (variableFullPathParts.length > 3) {
                    ...
                    switch (typeof variableValue) {
                        case 'string': {
                            const unEscapedVariableValue = (0, utils_1.handleEscapeCharacters)(variableValue, true);
                    ...
                    if (variableObj) {
                        ...
                        variableValue = (0, utils_1.handleEscapeCharacters)(typeof variableObj === 'object' ? JSON.stringify(variableObj) : variableObj, false);
                    }
                }
                if (isAcceptVariable) {
                    variableDict[`{{${variableFullPath}}}`] = variableValue;
...
const resolveVariables = async (reactFlowNodeData, reactFlowNodes, question, chatHistory, flowConfig, uploadedFilesContent, availableVariables = [], variableOverrides = []) => {
    ...
    logger_1.default.debug(`========= Start of resolveVariables start of resolveVariables for ${reactFlowNodeData.name} File: /Users/brianbagdasarian/fw-dev/Flowise/packages/server/src/utils/index.ts line 1009 =========`);
    logger_1.default.debug(JSON.stringify(flowNodeData));
    logger_1.default.debug(`========= End of resolveVariables end of resolveVariables for ${reactFlowNodeData.name} File: /Users/brianbagdasarian/fw-dev/Flowise/packages/server/src/utils/index.ts line 1009 =========`);
    const types = 'inputs';
    const getParamValues = async (paramsObj) => {
        logger_1.default.debug(`========= Start of getParamValues start of getParamValues for ${reactFlowNodeData.name} File: /Users/brianbagdasarian/fw-dev/Flowise/packages/server/src/utils/index.ts line 1017 =========`);
        logger_1.default.debug(JSON.stringify(paramsObj));
        logger_1.default.debug(`========= End of getParamValues end of getParamValues for ${reactFlowNodeData.name} File: /Users/brianbagdasarian/fw-dev/Flowise/packages/server/src/utils/index.ts line 1017 =========`);
        for (const key in paramsObj) {
            logger_1.default.debug(`========= Start of getParamValues for ${key} File: /Users/brianbagdasarian/fw-dev/Flowise/packages/server/src/utils/index.ts line 1017 =========`);
            logger_1.default.debug(JSON.stringify(paramsObj[key]));
            logger_1.default.debug(`========= End of getParamValues for ${key} File: /Users/brianbagdasarian/fw-dev/Flowise/packages/server/src/utils/index.ts line 1017 =========`);
...
const isStartNodeDependOnInput = (startingNodes, nodes) => {
    for (const node of startingNodes) {
        ...
        for (const inputName in node.data.inputs) {
            const inputVariables = (0, utils_1.getInputVariables)(node.data.inputs[inputName]);
...
const findAvailableConfigs = (reactFlowNodes, componentCredentials) => {
    ...
    for (const flowNode of reactFlowNodes) {
        logger_1.default.debug('========= Start of findAvailableConfigs for flowNode =========');
        ...
        logger_1.default.debug('========= End of findAvailableConfigs for flowNode =========');
        for (const inputParam of flowNode.data.inputParams) {
            ...
            if (inputParam.type === 'file') {
                ...
                for (const name of inputParam.credentialNames ?? []) {
                    logger_1.default.debug('========= Start of findAvailableConfigs for credential =========');
                    logger_1.default.debug('name', name);
                    logger_1.default.debug('componentCredentials', componentCredentials);
                    logger_1.default.debug('========= End of findAvailableConfigs for credential =========');
                    if (Object.prototype.hasOwnProperty.call(componentCredentials, name)) {
                        const inputs = componentCredentials[name]?.inputs ?? [];
                        logger_1.default.debug('========= Start of findAvailableConfigs for inputs =========');
                        logger_1.default.debug('inputs', JSON.stringify(inputs));
                        logger_1.default.debug('========= End of findAvailableConfigs for inputs =========');
...
const getEncryptionKey = async () => {
    try {
        logger_1.default.debug('========= Start of getEncryptionKey =========');
        ...
        logger_1.default.info(`[server]: Encryption key: ${encryptionKey}`);
        logger_1.default.debug('========= End of getEncryptionKey =========');
        return encryptionKey;
    }
    catch (error) {
        // Log the error and rethrow
        logger_1.default.error(`Error retrieving encryption key: ${error}`);
...
const encryptCredentialData = async (plainDataObj) => {
    try {
        ...
        const { storeSecret } = await Promise.resolve().then(() => __importStar(require('../services/secrets')));
        // Generate a unique ID for the credential
        const credentialId = (0, crypto_1.randomBytes)(12).toString('hex');
        // Store in Supabase using the secrets service
        const secretId = await storeSecret(`Credential_${credentialId}`, 'credential', plainDataObj, { credentialId });
        return secretId;
    }
    catch (error) {
        logger_1.default.error(`Error encrypting credential data: ${error}`);
        throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to encrypt credential data. Please ensure the ENCRYPTION_KEY is set in platform settings.');
    }
};
...
const decryptCredentialData = async (encryptedData, componentCredentialName, componentCredentials) => {
    try {
        logger_1.default.debug(`========= START decryptCredentialData (server) =========`);
        ...
        logger_1.default.debug(`componentCredentials provided: ${componentCredentials ? 'yes' : 'no'}`);
        // Get the secret from Supabase
        logger_1.default.debug(`Importing getSecret function`);
        const { getSecret } = await Promise.resolve().then(() => __importStar(require('../services/secrets')));
        logger_1.default.debug(`Calling getSecret with ID: ${encryptedData}`);
        const plainDataObj = await getSecret(encryptedData);
        logger_1.default.debug(`getSecret returned data: ${JSON.stringify(plainDataObj)}`);
        if (componentCredentialName && componentCredentials) {
            logger_1.default.debug(`Redacting credential with password type for ${componentCredentialName}`);
            const redactedData = (0, exports.redactCredentialWithPasswordType)(componentCredentialName, plainDataObj, componentCredentials);
            logger_1.default.debug(`Redacted data: ${JSON.stringify(redactedData)}`);
            logger_1.default.debug(`========= END decryptCredentialData (server) =========`);
            return redactedData;
        }
        ...
        logger_1.default.error(`Error in decryptCredentialData: ${(0, utils_2.getErrorMessage)(error)}`);
        logger_1.default.debug(`Full error: ${JSON.stringify(error)}`);
        logger_1.default.debug(`Stack trace: ${error.stack}`);
        logger_1.default.debug(`========= END decryptCredentialData (server) with error =========`);
        throw new Error(`Error getting credential data: ${(0, utils_2.getErrorMessage)(error)}`);
    }
};
...
const getMulterStorage = () => {
    ...
    if (storageType === 's3') {
        const s3Client = (0, storageUtils_1.getS3Config)().s3Client;
        const Bucket = (0, storageUtils_1.getS3Config)().Bucket;
